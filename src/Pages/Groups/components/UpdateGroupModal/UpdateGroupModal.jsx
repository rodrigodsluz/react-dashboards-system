...
function UpdateGroupModal({ group, onClose }) {
  const userInfo = useSelector((state) => state.User.user, shallowEqual);
  const uniqueGroup = useSelector((state) => state.Groups.uniqueGroup);
  ...
  const SUCCESS_UPDATE = 'Tudo pronto! Seu grupo foi atualizado com sucesso.';
  ...
  /**
   * @function handleGetStatusByGroup
   * @description Retorna os dados do grupo aberto
   */

  const handleGetStatusByGroup = useCallback(async () => {
    await dispatch.Groups.loadGroupByIdAsync(group?.id);
  }, [group]);

  /**
   * @function handleGetProductsInGroup
   * @description Salva o ID, áreas e status do grupo
   */

  const handleGetProductsInGroup = useCallback(() => {
    if (uniqueGroup) {
      setGroupId(uniqueGroup.id);
      setSelectedProducts(uniqueGroup.product_groups);
      setSelectedStatusByProducts(uniqueGroup.status_groups);
    }
  }, [group, uniqueGroup]);
  ...
  /**
   * @function handleChanceGroup
   * @description Seta os valores nos inputs de nome
   * e descrição ao trocar de grupo
   */

  const handleChanceGroup = useCallback(() => {
    if (group) {
  ...
  /**
   * @function handleSubmitUpdateGroup
   * @description Trata as informações de áreas que estiverem vazias
   * e atualiza o grupo
   */

  const handleSubmitUpdateGroup = useCallback(async () => {
    try {
      setLoading(true);
      ...
      await dispatch.Groups.updateGroupByIdAsync(payload);
    ...
    <Container>
      <Header spaceBetween>
        ...
        <Typography fontSize="25px" vertical="40px" horizontal="10px">
          <Title> INFORMAÇÔES DO GRUPO</Title>
          {' '}
        </Typography>
      </Header>
            ...
            <>
              ...
              {/* <WrapperCheckBox>
                <input
                  ...
                  value={product.name}
                />
                <label htmlFor={product.name}> Possui acesso a todos </label>
              </WrapperCheckBox> */}
              ...
              {/* <form onSubmit={(event) => handleAddStatusInArray(event)}>
                <Input
                  ref={inputEl}
                  type="text"
                  name={`${product.name}group`}
                  list={`${product.name}groups`}
              ...
              <DataList id={`${product.name}groups`}>
                {allStatusByProducts
                  && allStatusByProducts[product.name]?.map((elem) => (
      ...
      <ContainerButton>
        <PrimaryButton
          onClick={handleSubmitUpdateGroup}
          loading={loading}
          disabled={loading}
        >
          Atualizar grupo
